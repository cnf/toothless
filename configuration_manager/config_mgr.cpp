...
ConfigManager::ConfigManager() { snprintf(register_topic_, 30, "%s%s%s", kTopicConfig, TOPIC_DOT, kTopicRegister); }
...
void ConfigManager::Start() {
  ...
  xTaskCreatePinnedToCore(CfgMngrShim,        // Task function
                          ...
                          3072,               // Stack size of task, in bytes
...
void ConfigManager::Setup() {
  FLOG_DEBUG("Listening on %s, %s", kTopicConfig, kTopicIncConfig);
  config_sub_ = ps_new_subscriber(10, PS_STRLIST(kTopicConfig, kTopicIncConfig));
...
void ConfigManager::Loop() {
  ps_msg_t *msg = NULL;
  TopicParts parts;
  for ((msg = ps_get(config_sub_, 0)); msg != NULL; (msg = ps_get(config_sub_, 0))) {
...
bool ConfigManager::AddNamespace(std::string name_space) {
  if (std::find(nvs_namespaces_.begin(), nvs_namespaces_.end(), name_space) != nvs_namespaces_.end()) {
    return true;
  }
  nvs_namespaces_.push_back(name_space);
  std::sort(nvs_namespaces_.begin(), nvs_namespaces_.end());
...
bool ConfigManager::WriteNamespaces() {
  ...
  std::string buff =
      std::accumulate(nvs_namespaces_.begin(), nvs_namespaces_.end(), std::string(),
...
void GetSettings(std::shared_ptr<SettingsMap> &config, const char *name) {
  ...
  snprintf(r_topic, sizeof(r_topic), "$r.config.%u", ++rt_counter);
  ...
  time_t timer = portability::Millis() + 10000; // wait for 10 seconds
