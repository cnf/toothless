...
lv_obj_t* DryerScreen::Create() {
  ...
  _screen = lv_obj_create(NULL);
  lv_obj_set_style_bg_color(_screen, lv_color_hex(0x000033), 0);  // lv_color_black(), 0);
...
esp_err_t DryerScreen::Temperature() {
  lv_obj_t* temp_container = lv_obj_create(_labels->left);
  ...
  lv_obj_set_size(temp_container, lv_pct(100), lv_pct(100));
...
void DryerScreen::TimerHandler(lv_event_t* e) {
  ...
  TimeRollerContext ctx{.parent_screen = obj->_labels->right,
...
void DryerScreen::HeaterLED(lv_obj_t* parent) {
  ...
  lv_obj_add_flag(_labels->heater_led, LV_OBJ_FLAG_HIDDEN);  // Hide until we add more temps
}
